...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _fetchStatus() async {
    try {
      ...
      if (!mounted) return;
      setState(() { _status = s; _loadingStatus = false; });
  ...
  Future<void> _sendCommand(GpsCommand cmd) async {
    setState(() { _sendingCmd = cmd; _cmdResult = null; });
    final result = await SmsService.sendCommand(cmd, widget.session.simNumber);
    if (!mounted) return;
    setState(() {
      _sendingCmd = null;
  ...
  Future<void> _openSupport() async {
    ...
    final uri = Uri.parse('https://wa.me/$kSupportPhone?text=${Uri.encodeComponent(msg)}');
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(widget.session.vehicleName.isNotEmpty
              ? widget.session.vehicleName : 'Mi vehículo',
        ...
        actions: [
          IconButton(
            ...
            onPressed: _fetchStatus, tooltip: 'Actualizar',
          ),
          IconButton(
            icon: const Icon(Icons.support_agent, color: Color(0x66F0F6FF)),
            onPressed: _openSupport, tooltip: 'Soporte WhatsApp',
          ),
          IconButton(
      ...
      body: Column(children: [
        ...
        if (_cmdResult != null)
          Container(
            ...
            decoration: BoxDecoration(
              color: (_cmdSuccess ? AppTheme.teal : AppTheme.red).withOpacity(0.08),
              borderRadius: BorderRadius.circular(10),
              border: Border.all(
                color: (_cmdSuccess ? AppTheme.teal : AppTheme.red).withOpacity(0.2)),
            ),
            child: Row(children: [
              Icon(
                _cmdSuccess ? Icons.check_circle_outline : Icons.error_outline,
                color: _cmdSuccess ? AppTheme.teal : AppTheme.red, size: 16),
              const SizedBox(width: 8),
              Expanded(child: Text(_cmdResult!,
                style: TextStyle(
        ...
        Container(
          ...
          child: Column(children: [
            // Fila 1
            Row(children: [
              _CmdBtn(cmd: GpsCommand.locate,       sending: _sendingCmd, onTap: _sendCommand),
              const SizedBox(width: 8),
              _CmdBtn(cmd: GpsCommand.moveAlert,    sending: _sendingCmd, onTap: _sendCommand),
              const SizedBox(width: 8),
              _CmdBtn(cmd: GpsCommand.speedAlert,   sending: _sendingCmd, onTap: _sendCommand),
            ]),
            const SizedBox(height: 8),
            // Fila 2
            Row(children: [
              _CmdBtn(cmd: GpsCommand.stopEngine,   sending: _sendingCmd, onTap: _sendCommand, danger: true),
              const SizedBox(width: 8),
              _CmdBtn(cmd: GpsCommand.resumeEngine, sending: _sendingCmd, onTap: _sendCommand),
              const SizedBox(width: 8),
              _CmdBtn(cmd: GpsCommand.online,       sending: _sendingCmd, onTap: _sendCommand),
            ]),
          ]),
        ),
      ]),
    ...
    return FlutterMap(
      ...
      children: [
        ...
        if (status != null)
          MarkerLayer(markers: [
            Marker(
              point: center, width: 40, height: 40,
...
class _KpiRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        _Kpi(label: 'Velocidad',
            value: '${status.speed.toInt()} km/h',
            icon: Icons.speed, color: AppTheme.teal),
        const SizedBox(width: 8),
        _Kpi(label: 'Geocerca',
        ...
        _Kpi(label: 'Estado',
            ...
            icon: Icons.circle, color: status.statusColor),
      ]),
    );
  }
}
...
class _CmdBtn extends StatelessWidget {
  ...
  const _CmdBtn({
    required this.cmd, required this.sending,
